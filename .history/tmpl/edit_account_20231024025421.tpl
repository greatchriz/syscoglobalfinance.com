    ...
    <div class="col-12 mb-30">
      <div class="create_wrapper mw-100">
              ...
              >
                ...
                {include file="edit_account_input.tpl" label="Account Name:" userinfo="{$userinfo.username}" type="text" disabled="disabled"  value="{$userinfo.username}"}

                {include file="edit_account_input.tpl" label="Registration date:" userinfo="{$userinfo.date_register}" type="text" value="{$userinfo.date_register}" disabled="disabled"}

                {include file="edit_account_input.tpl" name="fullname" label="Full Name:" userinfo="{$userinfo.name}" type="text" value="{$userinfo.name}" disabled=""}

                {include file="edit_account_input.tpl" name="password" label="New Password:" type="password" value="" disabled=""} 


                {include file="edit_account_input.tpl" name="password2" label="Retype Password:" type="password" value="" disabled=""}

                {foreach item=ps from=$pay_accounts}
                  <h6>{$ps.name}</h6>
                  {if $settings.usercanchangeegoldacc == 0 && $ps.account != ''}
                    <h6>{$ps.account|escape:html}</h6>
                  {else}

                    {include file="edit_account_input.tpl" name="pay_account[{$ps.id}]" label="Your {$ps.name} acc no:"
                    type="text" value="{$frm.pay_account[$ps.id]|default:$ps.account|escape:html}"
                    disabled="{$settings.usercanchangeegoldacc == 0 && $ps.account != ''}"
                ...
                {foreach item=p from=$mpay_accounts}
                  {foreach item=ps from=$p.accounts}
                    ...
                    {include file="edit_account_input.tpl" name="pay_account[{$p.id}][{$ps.name|escape:html}]" label="Your
                {$p.name} {$ps.name}:" type="text"
                    value="{$frm.pay_account[$p.id][$ps.name]|default:$ps.value|escape:html}"
                    disabled="{$settings.usercanchangeegoldacc == 0 && $ps.value != ''}" data-validate="{$ps.validate.func}"
                    data-validate-{$ps.validate.func}="{$ps.validate[$ps.validate.func]}"
                    data-validate-notice="{$ps.validate.notification|escape:html}"}
                  {/foreach}
                {/foreach}
                ...
                {include file="edit_account_input.tpl" name="email" label="Your E-mail address:" type="text" value='{$userinfo.email|escape:"quotes"}'
                disabled="{$settings.usercanchangeemail != 1}"}
...
{* 
                ...
                >
                  ...
                  <tr>
                    ...
                    <td><input
                        type=text
                        name=fullname
                        value='{$userinfo.name|escape:"quotes"}'
                        class=inpts
                        size=30
                      >
                  </tr>
                  {if $settings.use_user_location}
                    <tr>
                      ...
                      <td><input
                          type=text
                          name=address
                          value="{$userinfo.address|escape:"quotes"}"
                          class=inpts
                          size=30
                        ></td>
                    </tr>
                    <tr>
                      ...
                      <td><input
                          type=text
                          name=city
                          value="{$userinfo.city|escape:"quotes"}"
                          class=inpts
                          size=30
                        ></td>
                    </tr>
                    <tr>
                      ...
                      <td><input
                          type=text
                          name=state
                          value="{$userinfo.state|escape:"quotes"}"
                          class=inpts
                          size=30
                        ></td>
                    </tr>
                    <tr>
                      ...
                      <td><input
                          type=text
                          name=zip
                          value="{$userinfo.zip|escape:"quotes"}"
                          class=inpts
                          size=30
                        ></td>
                    </tr>
                        ...
                        >
                          ...
                          {section name=c loop=$countries}
                            <option {if $countries[c].name eq $userinfo.country}selected{/if}>
                              {$countries[c].name|escape:"quotes"}
                            </option>
                          {/section}
                        </select>
                      </td>
                  ...
                  {foreach item=ps from=$pay_accounts}
                    <tr>
                      ...
                      <td>{if $settings.usercanchangeegoldacc == 0 && $ps.account != ''}{$ps.account|escape:html}
                      {else}<input
                            type=text
                            class=inpts
                            size=30
                            name="pay_account[{$ps.id}]"
                            value="{$frm.pay_account[$ps.id]|default:$ps.account|escape:html}"
                            data-validate="{$ps.validate.func}"
                            data-validate-{$ps.validate.func}="{$ps.validate[$ps.validate.func]}"
                            data-validate-notice="{$ps.validate.notification|escape:html}"
                        >{/if}</td>
                  ...
                  {foreach item=p from=$mpay_accounts}
                    {foreach item=ps from=$p.accounts}
                      <tr>
                        ...
                        <td>{if $settings.usercanchangeegoldacc == 0 && $ps.value != ''}{$ps.value|escape:html}
                        {else}<input
                              type=text
                              class=inpts
                              size=30
                  ...
                  <tr>
                    ...
                    <td>{if $settings.usercanchangeemail == 1}<input
                          type=text
                          name=email
                  ...
                  {if $settings.use_rcb}
                    <tr>
                      ...
                      <td>{if $userinfo.rcb_lock}{$userinfo.rcb}%
                      {else}<input
                            type=text
                            class=inpts
                            size=30
                            name="rcb"
                            value="{$frm.rcb|default:$userinfo.rcb|number_format:2}"
                        >{/if}</td>
                    </tr>
                  {/if}
                  {if $userinfo.admin_auto_pay_earning == 1}
                    <tr>
                      <td colspan=2><input
                          type=checkbox
                          name=user_auto_pay_earning
                          value=1
                          {if $userinfo.user_auto_pay_earning == 1}checked{/if}
                        > Receive earnings directly to e-currency account</td>
                    </tr>
                  {/if}
                  {if $settings.use_transaction_code_edit_account == 1}
                    <tr>
                ...
                <div class="create_form_group col-sm-6 align-self-end">
                  <button type="submit" class="custom-button border-0">Change Account data</button>
              </div>
                {literal}
                      <script language=javascript>
